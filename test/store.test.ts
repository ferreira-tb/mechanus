...
import { Mechanus, storeToRefs } from '../src/mechanus';
import { computed, isReadonly, ref, readonly } from '../src/reactivity/ref';
import { MechanusStoreError } from '../src/errors';
...
test('store already defined', () => {
  ...
  expect(() => mech.define('store', () => ({ foo: ref('foo') }))).toThrow(
    MechanusStoreError
  );
});
