...
test('until to be (throw on timeout with reason)', async () => {
    ...
    const options = { timeout: 50, throwOnTimeout: true, timeoutReason: 'This is a reason' };
    await expect(until(foo).toBe('baz', options)).rejects.toThrowError(options.timeoutReason);
    expect(foo.value).toBe('bar');
    clearTimeout(timeout);
});
...
test('until to be instance of (timeout)', async () => {
    ...
    const options = { timeout: 50, throwOnTimeout: false };
    await expect(until(foo).toBeInstanceOf(Date, options)).resolves.not.toThrowError();
    clearTimeout(timeout);
});
...
test('until to be instance of (throw on timeout)', async () => {
    ...
    await expect(until(foo).toBeInstanceOf(Date, { timeout: 50 })).rejects.toThrowError();
