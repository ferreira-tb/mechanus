...
export type StoreRawValues<T = any> = {
	[K in keyof T]: T[K] extends MechanusRef
		? UnwrapRef<T[K]>
		: T[K] extends ReadonlyMechanusRef
		  ? UnwrapReadonlyRef<T[K]>
		  : T[K] extends MechanusComputedRef
		    ? UnwrapComputedRef<T[K]>
		    : T[K] extends StoreAction
		      ? T[K]
		      : never;
};
...
export type MechanusStore<T = any> = StoreRawValues<T> & {
	...
	$raw<U extends MechanusStoreRawOptions>(
		options?: U
	): U['actions'] extends true
		? StoreRawValues<T>
		: StoreRawValuesWithoutActions<T>;
};
...
export type UseStorePatchFn<T extends StoreRefs> =
	| (() => MaybePromise<StorePartialState<T> | null>)
	| null;
export type DefineStoreReturn<T extends StoreRefs> = (
	patchFn?: UseStorePatchFn<T>
) => MechanusStore<T>;
...
export class Mechanus {
		...
		return new Proxy(store, {
			get(target, key) {
				if (typeof key !== 'string') {
					...
					return (partialState: Partial<StoreRawValues<T>>) =>
						self.#patch(store, partialState);
				} else if (key === '$raw') {
					return (options?: MechanusStoreRawOptions) =>
						self.#raw(store, options);
				} else if (key in target) {
					const item = Reflect.get(target, key);
					return unref(item);
				}

				throw new MechanusStoreError(
					`Reactive object does not have property "${String(key)}".`
				);
			},
			set(target, key, value) {
				if (typeof key !== 'string') {
					...
					if (isRef(item)) {
						...
						throw new MechanusStoreError(
							`Cannot set value of non-ref property "${key}".`
						);
					}
				}
	...
	#patch<T extends StoreRefs, U extends MechanusStore<T>>(
		store: T,
		partialState: Partial<U>
	): void {
		for (const [key, value] of Object.entries(partialState) as [
			keyof U,
			U[keyof U]
		][]) {
	...
	#raw<T extends StoreRefs>(
		store: MechanusStore,
		options: MechanusStoreRawOptions = {}
	): StoreRawValues<T> {
	...
	/** Define a new store. */
	public define<T extends StoreRefs>(
		name: string,
		refs: T
	): DefineStoreReturn<T>;
	public define<T extends StoreRefs>(
	...
	public define<T extends StoreRefs>(
		...
		const storeRefs =
			typeof storeSetupOrRefs === 'function'
				? storeSetupOrRefs()
				: storeSetupOrRefs;
	 ...
	 */
	public use<T extends StoreRefs>(
		name: string,
		patchFn: UseStorePatchFn<T> = null
	): MechanusStore<T> {
